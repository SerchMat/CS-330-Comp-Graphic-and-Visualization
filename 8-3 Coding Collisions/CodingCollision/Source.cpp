...
public:
	...
	// Function for the Reflect of the Balls with the Bricks
	void CheckCollision(Brick* brk)
	{
		...
		// Reflect the Balls Once hit the Brick
		...
		// Reaction when the Brick is DEstroy
		else if (brk->brick_type == DESTRUCTABLE && brk->onoff == ON
			&& brk->health > 0 && onoff == ON)
		{
			if ((x > brk->x - brk->width && x <= brk->x + brk->width) && (y > brk->y - brk->width && y <= brk->y + brk->width))
			{
				brk->health--;
...
int main(void) {
	...
	GLFWwindow* window = glfwCreateWindow(580, 580, "Brick Breakers", NULL, NULL); // Name of the Screen
	if (!window) {
		glfwTerminate();
		exit(EXIT_FAILURE);
	}
