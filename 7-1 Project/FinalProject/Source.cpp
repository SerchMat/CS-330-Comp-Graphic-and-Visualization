...
{
    // Windows Name
    const char* const WINDOW_TITLE = "Light & Objects Final Project";
    const int STRIDE = 7;
    ...
    {
        // Vertex Array Object
        GLuint ruleVao, planeVao, cylinderVao, cubeVao, logoVao, markerVao;

        // Vertex Buffer Object
        GLuint ruleVbo, planeVbo, cubeVbo, logoVbo, markerVbo;

        // Indices of the Mesh
        GLuint ruleVertices, planeVertices, cylinderVertices, cubeVertices, logoVertices, markerVertices;
        GLuint cylinderVbos[2], cylinder2Vbos[3];
    ...
    GLuint gMarkerProgramId;    /* Marker */
    ...
    glm::vec3 gLightScale(1.5f);

    // Wall Plug (3D RECTANGLE)
    glm::vec3 gPlugPosition(0.0f, -0.8f, -1.0f);
    ...
    // Rule (RECTANGLE)
    glm::vec3 gRulePosition(0.0f, -1.2f, -0.3f);
    ...
    // Speaker (CYLINDER)
    glm::vec3 gSpeakerPosition(0.0f, -1.0f, 1.0f);
    glm::vec3 gSpeakerScale(3.0f);

    // Marker (CYLINDER)
    glm::vec3 gMarkerPosition(0.0f, -1.02f, 2.5f);
    glm::vec3 gMarkerScale(3.0f);

    glm::mat4 rotationMarkerX = glm::rotate(0.01f, glm::vec3(1.0, 0.0f, 0.0f));
    glm::mat4 rotationMarkerY = glm::rotate(7.0f, glm::vec3(0.0, 1.0f, 0.0f));
    glm::mat4 rotationMarkerZ = glm::rotate(1.58f, glm::vec3(0.0, 0.0f, 1.0f));

    // Logo (CUBE)
    glm::vec3 gLogoPosition(0.4f, -0.7f, 0.95f);
...
// Create Marker
void UCreateMarker(GLfloat verts[], GLushort indices[], int numSides, float radius, float halfLen);
...
int main(int argc, char* argv[])
{
    ...
    /* Marker */
    if (!UCreateShaderProgram(cubeVertexShaderSource, cubeFragmentShaderSource, gMarkerProgramId))
        return EXIT_FAILURE;
    ...
    // Marker
    texFilename = "marker.jpg";
    if (!UCreateTexture(texFilename, gMarkerPattern))
    {
        cout << "Failed to load texture " << texFilename << endl;
        return EXIT_FAILURE;
    }
    ...
    UDestroyShaderProgram(gMarkerProgramId);
...
void URender()
{
    ...
    glUseProgram(gProgramId);
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, gMarkerPattern);


    model = glm::translate(gMarkerPosition) * glm::scale(gMarkerScale) * rotationMarkerZ * rotationMarkerY;

    modelLoc = glGetUniformLocation(gMarkerProgramId, "model");
    viewLoc = glGetUniformLocation(gMarkerProgramId, "view");
    projLoc = glGetUniformLocation(gMarkerProgramId, "projection");
    ...
    glBindVertexArray(gMesh.markerVao);

    glDrawElements(GL_TRIANGLES, gMesh.markerVertices, GL_UNSIGNED_SHORT, NULL);
...
void UCreateMesh(GLMesh& mesh)
{
    ...
    // Activate and Fill the Marker
    UCreateMarker(verts, indices, NUM_SIDES, 0.08f, 0.40f);

    /* ******************* DRAW THE MARKER ******************* */
    const GLuint floatsPerVertex3 = 3;
    const GLuint floatsPerColor3 = 4;
    const GLuint floatsPerUV3 = 2;

    glGenVertexArrays(1, &mesh.markerVao);
    glBindVertexArray(mesh.markerVao);

    // Create Buffers
    glGenBuffers(2, mesh.cylinder2Vbos);

    // Activate the Buffer
    glBindBuffer(GL_ARRAY_BUFFER, mesh.cylinder2Vbos[0]);

    // Send Verter or Coordinations
    glBufferData(GL_ARRAY_BUFFER, sizeof(verts), verts, GL_STATIC_DRAW);

    mesh.markerVertices = sizeof(indices) / sizeof(indices[0]);

    // Activates the Buffer
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mesh.cylinder2Vbos[1]);

    // Send Vertex and Coordinations
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(indices), indices, GL_STATIC_DRAW);

    // Stride between Vertex and Coordinations
    GLint stride3 = sizeof(float) * (floatsPerVertex3 + floatsPerColor3);

    // Create Vertex Attribute Pointers
    glVertexAttribPointer(0, floatsPerVertex3, GL_FLOAT, GL_FALSE, stride2, 0);
    glEnableVertexAttribArray(0);

    glVertexAttribPointer(1, floatsPerColor3, GL_FLOAT, GL_FALSE, stride2, (char*)(sizeof(float) * floatsPerVertex3));
    glEnableVertexAttribArray(1);

    glVertexAttribPointer(2, floatsPerUV3, GL_FLOAT, GL_FALSE, stride2, (char*)(sizeof(float) * (floatsPerVertex3 + floatsPerColor3)));
    glEnableVertexAttribArray(2);
